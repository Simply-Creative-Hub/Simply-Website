...
mkdir -p apps/$APP_NAME/{mobile,web,desktop,backend} packages/shared
...
echo "🛠️ Configuring Tailwind for mobile..."
...
cd ../web
echo "🌐 Setting up Expo Web app (optional)"
npx create-expo-app . --template blank
rm -f package-lock.json && rm -rf node_modules
yarn install
...
echo "➡️  Start web app:      cd apps/$APP_NAME/web && yarn start"
